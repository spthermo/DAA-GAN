...
class Data(object):
    ...
    def __init__(self, images, masks, index, labels, phase, scanner, downsample=1):
        """
        Data constructor.
        ...
        if images.shape[-1] != masks.shape[-1] or images.shape[0] != masks.shape[0] or images.shape[-2] != masks.shape[-2]:
        ...
        if images.shape[0] != labels.shape[0]:
            raise ValueError('Different number of images and labels: %d vs %d' % (images.shape[0], labels.shape[0]))
        if images.shape[0] != phase.shape[0]:
            raise ValueError('Different number of images and phase: %d vs %d' % (images.shape[0], phase.shape[0]))
        ...
        log.info('Labels value range [%.1f, %.1f]' % (labels.min(), labels.max()))
        log.info('Phase value range [%.1f, %.1f]' % (phase.min(), phase.max()))
    ...
    def merge(self, other):
        ...
        assert self.labels.shape[1:] == other.labels.shape[1:], str(self.labels.shape) + ' vs ' + str(other.labels.shape)
        assert self.phase.shape[1:] == other.phase.shape[1:], str(self.phase.shape) + ' vs ' + str(other.phase.shape)

        self.images = np.concatenate([self.images, other.images], axis=0)
        self.labels = np.concatenate([self.labels, other.labels], axis=0)
        self.phase = np.concatenate([self.phase, other.phase], axis=0)
    ...
    def roi(self, l=None, r=None):
        log.info('Cropping images and masks to a ROI')
        if l is None or r is None:
            l, r = utils.image_utils.get_roi_dims([self.masks])
        self.images = self.images[:, l:r, l:r, :]
        self.masks = self.masks[:, l:r, l:r, :]
        self.l = l
        self.r = r

    def make_even_coords(self):
        log.info('Cropping extra pixels to make even coordinates')
        sh = self.images.shape
        if self.images.shape[1] % 2 > 0:
            self.images = self.images[:,0:sh[1]-1,:,:]
            self.masks = self.masks[:,0:sh[1]-1,:,:]
        if self.images.shape[2] % 2 > 0:
            self.images = self.images[:,:,0:sh[2]-1,:]
            self.masks = self.masks[:,:,0:sh[2]-1,:]
    ...
    def get_labels(self, vol):
        return self.labels[self.index == vol]

    def get_phase(self, vol):
        return self.phase[self.index == vol]
    ...
    def filter_by_scanner(self, scanner):
        ...
        self.labels  = self.labels[self.scanner == scanner]
        self.phase  = self.phase[self.scanner == scanner]
    ...
    def sample_per_volume(self, num, seed=-1):
        ...
        new_images, new_masks, new_scanner, new_index, new_labels, new_phase = [], [], [], [], []
        for vol in self.volumes():
            images  = self.get_images(vol)
            labels  = self.get_labels(vol)
            phase  = self.get_phase(vol)
            ...
            phase  = np.array([phase[i] for i in idx])
        ...
        self.phase  = np.concatenate(new_phase, axis=0)
    ...
    def sample_images(self, num, seed=-1):
        ...
        self.labels  = np.array([self.labels[i] for i in idx])
        self.phase  = np.array([self.phase[i] for i in idx])
    ...
    def sample(self, num, seed=-1):
        ...
        if num == 0 or len(volumes) == 0:
            ...
            self.labels  = np.zeros(shape=(0,) + self.labels.shape[1:])
            self.phase  = np.zeros(shape=(0,) + self.phase.shape[1:])
        ...
        self.labels  = np.concatenate([self.get_labels(v) for v in volumes], axis=0)
        self.phase  = np.concatenate([self.get_phase(v) for v in volumes], axis=0)
