...
def resample_ants(nii_file, nii_file_newres, new_res=(1.37, 1.37, 10, 1)):
    ...
    os.system('/Users/agis/bin/ants/bin/ResampleImage %d %s %s %s' %
              (len(new_res), nii_file, nii_file_newres, 'x'.join([str(r) for r in new_res])))


def normalise(array, min_value, max_value):
    array = (max_value - min_value) * (array - array.min()) / (array.max() - array.min()) + min_value
...
def swap_last_columns(array, col1, col2):
    '''
    This function exchanges the values between two columns in the last dimension of the array.
